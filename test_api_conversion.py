#!/usr/bin/env python3
"""
测试API转换功能
"""

import requests
import json
import time
import os
from pathlib import Path

def test_conversion():
    """测试文档转换"""
    base_url = "http://localhost:8000"
    
    # 确保输出目录存在
    output_dir = Path("/workspace/output")
    output_dir.mkdir(exist_ok=True)
    
    print("=== 测试文档转换API ===")
    
    # 1. 测试健康检查
    print("\n1. 测试健康检查...")
    try:
        response = requests.get(f"{base_url}/health")
        print(f"健康检查状态: {response.status_code}")
        print(f"响应: {response.json()}")
    except Exception as e:
        print(f"健康检查失败: {e}")
        return
    
    # 2. 测试Office文档转换
    test_files = [
        {
            "name": "Word文档",
            "input": "/workspace/test/智涌君.docx",
            "output": "/workspace/output/智涌君_api_test.md",
            "task_type": "office_to_markdown"
        },
        {
            "name": "Excel文档", 
            "input": "/workspace/test/人工智能竞赛训练平台 v20250629.xlsx",
            "output": "/workspace/output/人工智能竞赛训练平台_api_test.md",
            "task_type": "office_to_markdown"
        },
        {
            "name": "PowerPoint文档",
            "input": "/workspace/test/AI通识课程建设方案.pptx", 
            "output": "/workspace/output/AI通识课程建设方案_api_test.md",
            "task_type": "office_to_markdown"
        },
        {
            "name": "PDF文档",
            "input": "/workspace/test/服装识别需求描述.pdf",
            "output": "/workspace/output/服装识别需求描述_api_test.md", 
            "task_type": "pdf_to_markdown"
        }
    ]
    
    for i, test_file in enumerate(test_files, 1):
        print(f"\n{i}. 测试{test_file['name']}转换...")
        
        # 检查输入文件是否存在
        if not os.path.exists(test_file['input']):
            print(f"  输入文件不存在: {test_file['input']}")
            continue
            
        # 创建任务
        task_data = {
            "task_type": test_file['task_type'],
            "input_path": test_file['input'],
            "output_path": test_file['output'],
            "priority": "normal",
            "params": {"force_reprocess": True}
        }
        
        try:
            print(f"  创建任务: {test_file['input']} -> {test_file['output']}")
            response = requests.post(
                f"{base_url}/api/tasks",
                json=task_data,
                timeout=10
            )
            
            if response.status_code == 200:
                task_info = response.json()
                task_id = task_info['task_id']
                print(f"  任务创建成功，ID: {task_id}")
                
                # 等待任务完成
                print("  等待任务完成...")
                max_wait = 120  # 最多等待2分钟
                start_time = time.time()
                
                while time.time() - start_time < max_wait:
                    try:
                        status_response = requests.get(f"{base_url}/api/tasks/{task_id}")
                        if status_response.status_code == 200:
                            task_status = status_response.json()
                            status = task_status.get('status', 'unknown')
                            print(f"  任务状态: {status}")
                            
                            if status == 'completed':
                                print(f"  ✅ 任务完成成功!")
                                
                                # 检查输出文件
                                if os.path.exists(test_file['output']):
                                    file_size = os.path.getsize(test_file['output'])
                                    print(f"  输出文件: {test_file['output']} ({file_size} bytes)")
                                    
                                    # 显示文件前几行内容
                                    try:
                                        with open(test_file['output'], 'r', encoding='utf-8') as f:
                                            content = f.read()[:200]
                                            print(f"  内容预览: {content}...")
                                    except Exception as e:
                                        print(f"  读取文件错误: {e}")
                                else:
                                    print(f"  ⚠️ 输出文件不存在: {test_file['output']}")
                                break
                                
                            elif status == 'failed':
                                print(f"  ❌ 任务失败")
                                if 'error' in task_status:
                                    print(f"  错误信息: {task_status['error']}")
                                break
                                
                            elif status in ['pending', 'processing']:
                                time.sleep(5)  # 等待5秒后再检查
                            else:
                                print(f"  未知状态: {status}")
                                break
                        else:
                            print(f"  获取任务状态失败: {status_response.status_code}")
                            break
                    except Exception as e:
                        print(f"  检查任务状态错误: {e}")
                        break
                else:
                    print(f"  ⏰ 任务超时 (>{max_wait}秒)")
                    
            else:
                print(f"  创建任务失败: {response.status_code}")
                print(f"  响应: {response.text}")
                
        except Exception as e:
            print(f"  测试失败: {e}")
    
    print("\n=== 测试完成 ===")

if __name__ == "__main__":
    test_conversion()
