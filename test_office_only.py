...
def test_office_conversion():
    ...
    # 2. 测试Office文档转PDF（不转Markdown）
    test_files = [
        {
            "name": "Word文档转PDF",
            "input": "/workspace/test/智涌君.docx",
            "output": "/workspace/output/智涌君_office_test.pdf",
            "task_type": "office_to_pdf"
        },
        {
            "name": "Excel文档转PDF", 
            "input": "/workspace/test/人工智能竞赛训练平台 v20250629.xlsx",
            "output": "/workspace/output/人工智能竞赛训练平台_office_test.pdf",
            "task_type": "office_to_pdf"
        },
        {
            "name": "PowerPoint文档转PDF",
            "input": "/workspace/test/AI通识课程建设方案.pptx", 
            "output": "/workspace/output/AI通识课程建设方案_office_test.pdf",
            "task_type": "office_to_pdf"
        }
    ]
                        ...
                        if status_response.status_code == 200:
                            ...
                            if status == 'completed':
                                ...
                                if os.path.exists(test_file['output']):
                                    file_size = os.path.getsize(test_file['output'])
                                    print(f"  输出文件: {test_file['output']} ({file_size} bytes)")
                                else:
